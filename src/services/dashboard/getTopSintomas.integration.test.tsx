...
// src/services/dashboard/getTopSintomas.integration.test.tsx
...
describe('Integración: getTopSintomas → Dashboard', () => {
  beforeEach(() => {
    vi.mocked(getTopSintomas).mockResolvedValue(mockSintomas);
  });
  ...
  it('debe mostrar los síntomas adversos en el gráfico cuando el servicio responde', async () => {
    renderDashboard();

    expect(await screen.findByText('Miocarditis')).toBeDefined();
    expect(await screen.findByText('Anafilaxia')).toBeDefined();
  });
});
